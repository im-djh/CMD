...
class DataProcessor(object):
    ...
    def mask_points_and_boxes_outside_range(self, data_dict=None, config=None):
        ...
        return data_dict
    def remove_boxes_without_points(self,data_dict=None, config=None):
        if data_dict is None:
            return partial(self.remove_boxes_without_points, config=config)

        # if data_dict.get('points', None) is not None:
          
        #     data_dict['points'] = data_dict['points'][mask]
        points=data_dict['points']
        corners_lidar = box_utils.boxes_to_corners_3d(np.array(data_dict['gt_boxes']))
        num_gt = np.array(data_dict['gt_boxes']).shape[0]
